...
export function premiumSubscriptionSql(alias = "", nowSql = "datetime('now')") {
  ...
  return `((${status} IN ('active','grace','cancelled') `
    ...
    + `AND datetime(substr(${prefix}current_period_end,1,19)) > ${nowSql}) OR (`
...
export function premiumChildSubscriptionSql(alias = "", nowSql = "datetime('now')") {
  const prefix = alias ? `${alias}.` : "";
  return `(LOWER(TRIM(COALESCE(${prefix}status, ''))) IN ('active','grace') AND ${prefix}expires_at IS NOT NULL `
    + `AND datetime(substr(${prefix}expires_at,1,19)) > ${nowSql})`;
}
 ...
 */
export function premiumFamilyIdsSql(nowSql = "datetime('now')") {
  return `
    SELECT entitlement_fs.family_id AS family_id
      FROM family_subscription entitlement_fs
     WHERE ${premiumSubscriptionSql("entitlement_fs", nowSql)}
    UNION
    SELECT entitlement_cs.family_id AS family_id
      FROM subscriptions entitlement_cs
     WHERE ${premiumChildSubscriptionSql("entitlement_cs", nowSql)}
    UNION
 ...
 */
export function premiumFamilyEntitlementSql(familyAlias = "f", nowSql = "datetime('now')") {
  const familyPrefix = familyAlias ? `${familyAlias}.` : "";
  return `(${familyPrefix}id IN (${premiumFamilyIdsSql(nowSql)}))`;
}
