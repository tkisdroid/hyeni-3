/**
 * 자녀 사진 업로드 엔드포인트.
 * 흐름: 이미지 바이트를 R2(Worker proxy)에 PUT 한 뒤, 그 저장 경로(raw path)를 photo_url 로
 *       family_members 에 기록한다. 표시용 proxy URL 변환은 getMyFamily 의 enrichPhotos 담당이라
 *       저장은 항상 raw 경로로 한다.
 *
 * 백엔드 계약(hyeni-1 Worker):
 *  - PUT /api/storage/child-photos/{familyId}/{...}.jpg  (raw binary, Bearer, 가족 격리) → { path }
 ...
 * @param stamp 경로 유니크용(컴포넌트가 Date.now() 등을 넘김 — 유틸은 인자로만 받는다).
 */
export async function uploadChildPhoto(
  ...
  const path = `${familyId}/${memberId}-${stamp}.jpg`;
  await apiUploadChildPhoto(path, blob, blob.type || "image/jpeg");
...
/**
 * 페어링 위저드용: 아직 멤버가 없는 placeholder 아이 사진을 order 기반 경로로 업로드하고 경로를 반환.
 * (멤버 id 가 없으므로 child_order 를 키에 쓴다 — 이 경로를 setupFamily children[].photo_url 로 전달.)
 */
export async function uploadPlaceholderChildPhoto(
  ...
  const blob = dataUrlToBlob(dataUrl);
  const path = `${familyId}/child-${childOrder}-${stamp}.jpg`;
  await apiUploadChildPhoto(path, blob, blob.type || "image/jpeg");
  return path;
}
...
export interface DraftChildWithPhoto {
  ...
  colorHex?: string;
  colorIndex?: number;
  /** 미업로드 data:URL(선택). 있으면 order 기반 경로로 업로드 후 photo_url 로 저장. */
  photoDataUrl?: string;
}

/**
 * 위저드: 아이 placeholder 생성 + (있으면) 사진 업로드를 한 번에.
 * 각 사진을 order 기반 경로로 업로드하고 그 경로를 photo_url 로 setupFamily 에 넘긴다.
...
export async function setupChildrenWithPhotos(input: {
  ...
  const children = await Promise.all(
    input.children.map(async (c, i) => {
      ...
      if (c.photoDataUrl) {
        try {
          photoUrl = await uploadPlaceholderChildPhoto(input.familyId, order, c.photoDataUrl, `${stamp}-${i}`);
        } catch (e) {
