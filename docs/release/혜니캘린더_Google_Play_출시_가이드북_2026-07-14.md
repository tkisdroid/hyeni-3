...
친구 초대는 구독권이 아니라 양쪽 가족에 각각 AI 대화 50회를 한 번 지급하며 초대 가족 수 상한은 없다. 신규 가족 생성 72시간 경과와 첫 실제 위치 저장 후 48시간 유지가 모두 확인된 뒤 서버 cron만 지급하고, 본인·공동 보호자·기존 가족·중복 귀속·부정 이용은 제외한다. 위치 좌표·주소·자녀 이름은 추천 원장에 저장하지 않는다. 승인·환불 금융 정본 보존과 위치 확인자료 보존은 공개 법적 문구·운영 migration·전문 검토가 끝나기 전 출시 완료로 처리하지 않는다.
...
- 친구 초대는 양쪽 가족에 각각 AI 대화 50회를 한 번만 지급하며 초대 가족 수 상한은 없다. 신규 가족 72시간·첫 실제 위치 후 48시간 유지와 본인·공동 보호자·기존 가족·중복 귀속·부정 이용 제외 조건을 서버가 검증하고, 클라이언트 지급 endpoint를 만들지 않는다.
