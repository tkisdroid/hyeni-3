...
- [x] 지도 화면·장소 검색·역지오코딩·길찾기를 `FamilyMap`/`/api/maps/*` 공통 경계로 이관하고 `KR=Kakao`, 공식 커버리지의 저장 가능 ISO 248개국=Google, `ZZ`·비표준 지역 코드=미지원인 fail-closed 정책을 자동 검증
...
- [ ] Google 지도 allowlist는 저장 가능 ISO 국가에서 `KR`만 제외한 248개국을 활성화했다. `docs/operations/google-maps-release-readiness.md`의 국가별 time zone/DST·Routes OAuth 실호출·부모 PWA↔아이 Android E2E와 새 Android AAB를 완료하기 전 해외 위치·알림 전체 출시는 HOLD
