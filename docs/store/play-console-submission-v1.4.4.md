...
현재 판정: **코드 수정·검증 완료, 서명 AAB·Play 교체 전**
...
- 서명 AAB는 최종 clean source commit에서 사용자가 서명 비밀번호를 직접 입력한 뒤 새로 만든다. 기존 code 15 AAB를
  재사용하지 않는다.
- AAB 생성·manifest/source/certificate/16KiB 검증과 Play production 교체·fresh readback 전에는 제출 완료로
  표시하지 않는다.
