...
- 친구 초대는 구독권이 아니라 양쪽 가족에 각각 AI 대화 50회를 한 번씩 지급한다.
- 초대 가족 수 상한은 없다.
- 주 보호자만 `HYENI-` 접두의 80비트 Crockford 코드를 발급한다.
- 본인, 공동 보호자, 기존 가족, 중복 귀속과 그 밖의 부정 이용을 서버에서 막는다.
...
- 추가 매출: AI 30/80/200팩 + 양쪽 가족에 각각 AI 대화 50회를 한 번 지급하는 친구 초대 보상(초대 가족 수 상한 없음)
