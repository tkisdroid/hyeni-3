# 2026-09-12 이전 작업 이력 원문

이하는 기존 `CLAUDE.md`의 보존용 스냅샷입니다. 현재 작업 지침이나 운영 상태를 뜻하지 않습니다.
옛 링크와 경로는 당시 저장소 루트를 기준으로 해석합니다. 현재 지침은 루트 `AGENTS.md`를 사용합니다.
...
**v1.4.6/code 18 소개 개선·출시 준비(2026-09-07, 서명·Console 로그인 대기)**:
사용자가 앱 소개 개선·버전업·최종 업데이트 파일·Play 검토 요청까지 지시했다. 기존 main 수정·미추적 파일을
보존하고 `.worktrees/play-release-20260907` / `release/play-20260907`에 이번 8개 개선과 한국어 소개,
버전·회귀 검사를 모아 `6a155fbc3a16174e32d8429b11d544a0d740500b`로 커밋했다. main 병합·push는 하지 않았다.
출시 후보는 1.4.6/code 18이며 Play 최신 사용 code는 로그인 후 확인해야 한다. 소개는 학교 도착·준비물·수학
학습의 사용 이점을 먼저 설명하고 무료 위치와 프리미엄 실시간 위치를 구분한다. 정본은 해당 브랜치의
`store/global/source/ko-KR.json`·`docs/store/play-listing.md`; 전달용 문구·출시 노트는
`output/play-release-v1.4.6`에 있다. 다른 언어의 전체 설명 번역·검수 대기를 완료로 바꾸지 않았다.

출시 후보 앱 2,098/2,098·Worker 1,457/1,457, typecheck·i18n·build, Android 단위 186/186·lint·debug APK,
PWA 설치·오프라인·업데이트 검증이 통과했다. 첫 출시 후보 브라우저 검사는 웹 재빌드와 겹친 자산 404로
1개 화면만 실패했으나, 빌드가 끝난 고정 자산으로 재검사해 부모 44·아이 15개 화면과 추가 흐름 전체 문제
0건을 확인했다. 최종 증거는 `%TEMP%/hyeni-146-browser-stable/report.json`이다.
서명 사전 점검은 승인 키·인증서·지도 설정·clean source를 확인했으며, Orca `혜니 1.4.6 서명` 터미널은
사용자의 키스토어/키 비밀번호 직접 입력을 기다린다. Chrome Play Console은 Google 로그인 단계다.
서명 AAB·Play 업로드·검토 요청은 아직 완료되지 않았다. Worker/D1·Pages·실기기 설치도 수행하지 않았다.

출시 worktree는 node_modules junction을 사용하므로 `NODE_OPTIONS=--preserve-symlinks`를 유지해야
`cap sync android`가 생성하는 상대 경로와 clean source가 보존된다. 실행 중인 서명 스크립트는 이 설정을
적용했고 커밋을 고정했으므로 입력 대기 중 출시 브랜치의 tracked 파일을 변경하지 않는다.

**전체 앱 조사 후 8개 개선 반영(2026-09-07, 로컬 수정·운영 미적용)**:
결제 공급자 예약과 위치 이력 보존 SQL은 호출자가 전달한 시각으로 구독 만료를 판정한다. RTDN 테스트의
과거 만료 fixture를 수정하고 만료 전·정각·직후 회귀를 추가했다. ID 로그인 오류는 입력값·비밀번호 포커스를
유지하면서 화면 오류 한 곳으로 표시한다. DataSync의 마지막 확인은 계정·가족 조회가 모두 성공한 실제
`dataUpdatedAt` 중 오래된 시각이며, 화면 재진입만으로 갱신하지 않는다. DaySummary 생성 결과는
가족·아이·날짜에 귀속하고 전환 뒤 늦게 도착한 응답은 현재 화면에 적용하지 않는다.

네이티브 언어 동기화 스케줄러를 독립 모듈로 분리해 테스트의 Vite 전체 로드를 제거했고, 아이 학습 문구를
반말로 통일했다. 브라우저 QA는 최신 학습 배너 문구, 로그인 오류 중복, 확인 시각 재진입·재조회,
하루 요약 아이·날짜 전환을 검증하며 비동기 화면은 준비 상태를 기다린다.

PWA는 한국어·모든 언어의 공통 부팅 문구를 사전 저장하고, 다른 언어의 순수 화면 카탈로그만 지연한다.
선택 언어는 첫 SW 제어권 이전에 이미 로드됐어도 SW가 활성화된 뒤 자동 저장하며, 푸시 API 지원 여부와
무관하게 동작한다. 같은 출처의 해시 정적 JS만 최대 200개·30일 보관한다. 초기 전체 JS 예산은
650,000 bytes, 사전 저장 전체 예산은 6MiB이며 최신 빌드는 초기 전체 JS 609,228 bytes,
사전 저장 414 URL·5,102.35KiB다(기존 484 URL·7,612.89KiB).

앱 2,098/2,098(105.7초, 최초 조사 593초), Worker 1,456/1,456, 앱·Worker typecheck,
i18n verify, production build 통과. PWA QA는 일본어 첫 렌더·푸시 미지원·선택 언어 8개 청크 자동 저장·
오프라인 재시작·온라인 문서 신선도·업데이트·정리 모두 문제 0건이다.
최종 격리 브라우저 QA도 부모 44·아이 15개 화면과 추가 동작 검사 전체 문제 0건이며,
`%TEMP%/hyeni-fix-browser-complete-20260907/report.json`에 결과를 보관했다.
증거: `%TEMP%/hyeni-fix-app-final3.log`, `hyeni-fix-worker-complete.log`,
`hyeni-fix-pwa-complete-20260907/report.json`. 기존 수정·미추적 파일은 보존했으며,
커밋·푸시·Pages/Worker/Play 배포·운영 D1·실기기 설치는 수행하지 않았다.
...
0대를 읽었고, 출시 이름·한국어 출시 노트·100% 국가/지역을 확인했다. 출시 노트는 검토 중 출시 세부정보에서
`혜니스터디 미니앱이 출시되어 이제 학습도 함께 할 수 있어요.`로 명확히 바꿨으며, 저장 후 별도 재제출 대기 없이
code 17이 계속 검토 중임을 fresh readback했다. 기존 code 16 검토를 최신 code 17로 대체해
...
반환했다. 이제 `POST /api/family/member/profile`가 API의 `null`을 D1 빈 문자열로 변환하며, 실제 스키마를 사용한
route 회귀가 이름·생일·빈 전화번호의 원자 저장을 검증한다.
...
사용자가 UTF-8 PowerShell 7 창에서 비밀번호를 직접 입력했고 값은 읽거나 저장·출력하지 않았다. clean source
`eda3907d96e0046804c2d39eb4c2c310f4b1dfeb`에서 만든 서명 AAB는
`artifacts/release-evidence/play-upload-v1.4.4-vc16-eda3907/hyeni-calendar-v1.4.4-vc16-eda3907.aab`,
13,060,516 bytes, SHA-256 `45e66db8d6a60cae1a9a4c33105f285c85886d92b4aea588425d0010a5343a91`이다.
manifest/source/certificate/web assets/16KiB 검증은 모두 GREEN이다. Play code 16 교체 제출·fresh lifecycle readback은
아직 완료 전이며 정본은 `docs/store/play-console-submission-v1.4.4.md`다. Worker·D1·Pages·
운영 계정·역할·페어링·세션·refresh token은 변경하지 않았다.
Play의 edge-to-edge 권고는 차단 오류가 아니다. 이 AAB는 target SDK 36으로 Android 15+ edge-to-edge가 이미 강제되고,
`viewport-fit=cover`·Capacitor 8.4.1 `SystemBars`·화면별 상하단 safe-area 처리가 있다. 구형 Android 동작까지 바꾸는
`EdgeToEdge.enable()`을 경고 제거만 위해 추가하지 않는다. Android 15/16의 세로·가로·cutout·키보드·제스처/3버튼
내비게이션 실기기 확인은 연결 기기가 없어 미완료다.
...
- **피드백 내구 접수 계약(2026-07-14)**: `/api/feedback`은 인증 필수이며 sender/family 정본을 서버에서 결정한다.
  UUID requestId 멱등키와 사용자별 시간당 5건 제한을 적용하고, Cloudflare Email Service보다 먼저
  `user_feedback(type='feature_feedback', status='queued')`를 저장한다. `FEEDBACK_EMAIL`은
  `feedback@hyenicalendar.com`에서 인증된 `tkisdroid@gmail.com` 한 곳으로만 보내며 실제 성공만 `sent`, 바인딩 미설정·실패는
...
## 2026-09-07 — v1.4.7(19) Google Play 심사 제출 및 한국어 소개 전면 교체

서명 소스 35ed585의 AAB를 production에 업로드하고 출시 노트·한국어 짧은 소개·상세 소개를 저장했다. 변경사항 3개를 전송한 뒤 Console의 '검토 중인 변경사항'을 확인했다. Google 사전 자동 검사 진행 상태이며 승인/공개 완료는 아직 미확인이다. 최종 문구·AAB·심사 접수 화면·해시 정본은 output/play-release-v1.4.7/제출결과.md 및 같은 폴더에 보존했다. 기존 v1.4.6 준비 문서보다 이 제출 결과가 최신이다. Worker/D1/Pages 배포·실기기 설치·main 병합·push는 이번에 수행하지 않았다.
