...
- ★**문구 밀도(2026-09-26 TK 제보 "텍스트가 너무 많다")**: 한 화면에서 같은 사실을 두 번 말하지 않는다. 행 부제는 제목을 반복하지
  ...
  14자 이상 버튼을 먼저 본다.
- ★**대화 화면도 하단 메뉴를 남긴다(2026-09-27 TK 제보 "갑자기 메뉴가 없어져 불편")**: 09-26 에는 입력줄 아래 메뉴가
  겹겹이 쌓여 보여 숨겼지만, 대화는 하단 탭 목적지라 누르자마자 메뉴가 사라지면 길을 잃는다. `AppShell` 은 메뉴를 항상 그리고
  `data-chat` 은 모양 표식으로만 쓴다. 입력줄은 메뉴 위 84px 에 붙고 입력줄 띠와 메뉴 바닥을 같은 색으로 이어 한 하단 영역으로
  보이게 한다. 부모 탭바는 **입력하는 동안(`:has(.mc-input:focus)`)만** 내려가고 입력줄이 키보드 위 바닥에 붙는다.
  아이 독은 SOS 가 있어 입력 중에도 남는다(안전 동선 우선). AI 친구 버튼은 대화에서 입력줄까지 피한다(bottomInset 232).
  회귀=`tests/memoComposerDesign.test.mjs`.
...
- ★**어른 글자 굵기(2026-09-27 TK 제보 "글씨가 전체적으로 얇다")**: 웹 글꼴을 싣지 않아 Android 기본 한글 글꼴이 쓰이는데,
  400 은 반투명 유리 위에서 가늘게 번져 보였다. `.hy-adult` 굵기는 제목 700 > 행 600 > 본문·보조 500 이고 입력 안내
  문구만 400 으로 값과 구분한다. 굵어진 제목 때문에 320px 에서 홈 헤더 설정 버튼이 잘려 ≤340px 은 칭찬하기를 그림만 둔다.
- ★**앱 글꼴 = Hyeni Sans(2026-09-27 TK "폰트를 바꾸거나 텍스트를 줄여 심플·프리미엄하게")**: Pretendard Variable 1.309 를
  굵기 400–800 으로 좁혀 Jua 와 같은 한글 빈도 구간 99조각(2.2MB, 조각 중앙값 22KB)으로 나눈 것이다. OFL 예약 글꼴 이름 때문에
  수정본(조각)은 내부 이름을 'Hyeni Sans' 로 바꾸고 저작권·라이선스는 그대로 둔다(`public/fonts/pretendard/OFL.txt`).
  선언 CSS(47KB)는 진입 CSS 예산 밖이라 `main.tsx` 가 첫 화면 뒤 동적 import 하고(swap), 조각은 프리캐시하지 않는다 —
  부모 홈 한 화면이 8조각·약 140KB 를 받는다. 재생성 = `scripts/fonts/slice-pretendard.py`(fonttools·brotli).
  ⚠️ 2MB 단일 파일은 여전히 싣지 않는다(`tests/initialRenderPerformanceContract`).
- ★**탭 렌즈는 묶음 한가운데(2026-09-27 TK "활성 메뉴가 위로 쏠린 느낌")**: 탭 내용 위아래 여백이 8/4 로 달라 대칭인 렌즈가
  아이콘+이름 묶음보다 위로 떠 보였다. 내용은 위아래 4px·간격 4px 로 대칭이고 렌즈는 가로로 긴 알약(≈1.3:1)이다.
  안 고른 탭 아이콘은 `--ph-ink-3` 로 가라앉혀 렌즈 쪽만 눈에 띄게 한다.
- ★**문구 줄이기(2026-09-27)**: 칩·버튼이 스스로 말하는 곳의 제목·도움말은 화면에서 뺀다 — 대화 "자주 쓰는 문구"는 칩 묶음의
  `aria-label` 로만 남고, AI 일정 음성·텍스트 탭의 "…날짜와 시간을 정리해요"는 바로 아래 CTA 와 같은 말이라 없앴다.
  조용한 시간의 안전 예외 문장·원격 청취 고지·사진 탭 개인정보 설명은 테스트로 고정된 결정이라 유지한다.
