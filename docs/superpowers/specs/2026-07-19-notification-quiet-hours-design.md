...
- 부모 홈과 안심리포트의 앱 사용 행에서 `시스템 자녀 보호 기능` 같은 비런처 시스템 컴포넌트를 제외한다.
- 배터리·네트워크·알림·위치, 화면시간·잠금해제, 정상 사용자 앱 사용 행은 유지한다.
- Android 수집 단계는 `FLAG_SYSTEM|FLAG_UPDATED_SYSTEM_APP`이면서 런처 진입점이 없는 앱을 제외하고,
  웹 표시 단계는 기존 저장 데이터의 확인된 시스템 표시명·패키지를 다시 제외한다.
- 순수 설명문은 공통 `hy-explain` 규칙으로 테두리·그림자를 제거하고 14px·행간 1.55·한국어 어절 보존을 적용한다.
- 두 문장 이상인 핵심 설명은 문장별 block으로 분리한다. 배경색·아이콘은 정보 묶음 인지를 위해 유지할 수 있다.
- 오류·경고·재시도·attention·SOS·차단·권한 정책 모달과 클릭 카드·데이터 요약에는 설명문 규칙을 일괄 적용하지 않는다.
- 역할 선택 인물 이미지는 기존 `object-position:center top`을 보존하고, 약관 링크 실제 조작 영역은 44px 이상으로 만든다.
- `/perm-denied`는 제목·한 문장·설정 경로·두 CTA의 압축 구조를 유지하며 아이콘은 승인된 16/24px 척도를 쓴다.
...
11. A17 부모모드에만 `adb install -r`; 현재 계정·세션을 유지
12. A17 부모 UI와 A17 계측 테스트·브라우저 아이 역할 화면으로 서로 다른 quiet 시간, 일반 알림 억제, 안전 예외, 지연 재생 없음 교차 검증
13. 테스트 데이터와 임시 설정 원복
14. 두 저장소의 의도한 파일만 커밋·머지·푸시

refresh 토큰은 읽거나 회전하지 않는다. 연결 해제된 razr와 검증 제외 상태인 S25는 설치·실행·로그·세션 조회를 포함해 조작하지 않는다.
...
- 안전지표의 최근·많이 쓴 앱에 시스템 자녀 보호 컴포넌트가 보이지 않고 정상 사용자 앱은 유지된다.
- 일반 설명문은 무테두리 공통 타이포그래피와 문장 단위 줄바꿈을 사용하며 의미 있는 상태 경계는 유지된다.
- 웹·Worker·Android 자동 회귀와 A17 단독 실기기 검증이 모두 통과하며 razr·S25는 미조작 상태를 유지한다.
