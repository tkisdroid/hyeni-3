...
check("운영 Google 국가 allowlist 비어 있음", /GOOGLE_MAP_RELEASE_COUNTRIES:\s*readonly string\[\]\s*=\s*\[\]/u.test(read("shared/mapPolicy.ts")));
...
const result = {
  status: failedChecks.length > 0 ? "FAIL" : releaseMode ? "HOLD" : "READY_FOR_CREDENTIAL_INPUT",
  checks,
