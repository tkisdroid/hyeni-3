...
// 플랜 라벨은 billing namespace 의 message id 로 들어오므로 두 카탈로그를 함께 싣는다.
...
const koIntl = createIntl(
  { locale: "ko", messages: { ...koParent, ...koBilling } },
  createIntlCache(),
) as IntlShape;
...
test("무료 가족은 프리미엄 혜택을 확인하는 카드로 안내한다", () => {
  assert.deepEqual(resolveParentHomeSubscriptionCard({
    ...
    isError: false,
    isPremium: false,
    planLabelId: "billing.subscription.plan.free" as const,
    isTrial: false,
    trialDaysLeft: null,
    periodEnd: null,
  }, koIntl, "ko"), {
...
test("프리미엄 가족은 구독 상품과 이용 종료일을 관리 카드에 표시한다", () => {
  assert.deepEqual(resolveParentHomeSubscriptionCard({
    ...
    isError: false,
    isPremium: true,
    planLabelId: "billing.subscription.plan.monthly" as const,
    isTrial: false,
    trialDaysLeft: null,
...
test("무료 체험 중인 가족은 남은 일수를 우선 표시한다", () => {
  assert.deepEqual(resolveParentHomeSubscriptionCard({
    ...
    isError: false,
    isPremium: true,
    planLabelId: "billing.subscription.plan.trial" as const,
    isTrial: true,
    trialDaysLeft: 3,
    periodEnd: new Date("2026-09-30T15:00:00.000Z"),
  }, koIntl, "ko"), {
...
test("캐시된 프리미엄 정본이 있으면 재조회 오류에도 관리 상태를 유지한다", () => {
  const view = resolveParentHomeSubscriptionCard({
    ...
    isError: true,
    isPremium: true,
    planLabelId: "billing.subscription.plan.premium" as const,
    isTrial: false,
    trialDaysLeft: null,
    periodEnd: null,
  }, koIntl, "ko");
