...
test("웹 결제 서버 오류 코드는 결제·대사·해지 상태를 숨기지 않는 안내로 변환한다", () => {
  ...
  assert.equal(
    webBillingRequestFailureMessage(error("web_billing_charge_failed")),
    "카드 승인을 완료하지 못했어요. 카드 상태를 확인해 주세요.",
  );
  assert.equal(
    webBillingRequestFailureMessage(error("web_billing_reconciliation_pending")),
    "결제 결과를 확인하고 있어요. 같은 주문을 다시 확인해 주세요.",
  );
  assert.equal(
    webBillingRequestFailureMessage(error("billing_provider_reconciliation_pending")),
    "결제 결과를 확인하고 있어요. 같은 주문을 다시 확인해 주세요.",
  );
  assert.match(
    webBillingRequestFailureMessage(error("billing_provider_conflict_refund_required")),
    /다른 스토어 구독.*환불 확인/,
  );
  assert.equal(
    webBillingRequestFailureMessage(error("web_billing_cancellation_unavailable")),
