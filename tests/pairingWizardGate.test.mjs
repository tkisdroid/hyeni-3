...
test("페어링 위저드는 가족·구독 정보 확정 전 다자녀 선택과 코드 생성을 막는다", () => {
  ...
  assert.match(source, /parent\.pairingWizard\.checking/);
  assert.equal(koParent["parent.pairingWizard.checking"], "확인 중");
});
...
test("페어링 코드 생성 직전에도 현재 티어의 아이 수 상한을 다시 검사한다", () => {
  ...
  assert.match(source, /plannedChildCount: existingChildCount \+ validChildren\.length/);
});
