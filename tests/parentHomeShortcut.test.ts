...
test("바로가기에 표시되는 이름은 실제 이동 대상과 같아야 한다", () => {
  ...
  const routes = home.slice(
    home.indexOf("const shortcutRoutes"),
    home.indexOf("const shortcutLabelIds"),
  );
  for (const shortcut of shortcuts) {
    ...
    assert.ok(routes.includes(`"${shortcut.label}":`), `${shortcut.label} 라우트 매핑 누락`);
  }
});
