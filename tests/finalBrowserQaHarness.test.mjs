...
test("최종 브라우저 QA는 부모·아이 출시 화면과 핵심 전환 계약을 빠짐없이 고정한다", () => {
  assert.equal(PARENT_BROWSER_QA_ROUTES.length, 44);
  ...
  assert.equal(CHILD_BROWSER_QA_ROUTES.length, 15);
  assert.ok(PARENT_BROWSER_QA_ROUTES.includes("study"));
  assert.ok(CHILD_BROWSER_QA_ROUTES.includes("study/learn"));
  ...
  assert.match(source, /저장됨 · 알림은 프리미엄에서 받아요/);
