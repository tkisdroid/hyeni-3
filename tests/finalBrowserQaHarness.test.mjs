...
test("최종 브라우저 QA는 부모·아이 출시 화면과 핵심 전환 계약을 빠짐없이 고정한다", () => {
  assert.equal(PARENT_BROWSER_QA_ROUTES.length, 45);
  assert.ok(PARENT_BROWSER_QA_ROUTES.includes("study-management"));
  assert.ok(PARENT_BROWSER_QA_ROUTES.includes("study-management/claim"));
  ...
  assert.match(source, /scenario\.authCase === "success"/);
  ...
  assert.match(source, /scope: "study-management-linked-mutations"/);
  assert.match(source, /scope: "study-claim-url-erased-one-shot"/);
});
