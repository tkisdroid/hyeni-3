...
test("메모 dayStamp 는 로컬 일자(yyyy-mm-dd)로 떨어진다", () => {
  const stamp = memoDayStamp("2026-07-09T15:32:18.633Z");
  assert.match(stamp, /^\d{4}-\d{2}-\d{2}$/);
  assert.equal(memoDayStamp("not-a-date"), "");
});

test("메모 날짜 구분선 라벨은 오늘/어제/그 외 날짜를 구분한다", () => {
  const now = new Date(2026, 6, 10, 12, 0, 0); // 2026-07-10 금요일
  assert.equal(formatMemoDayLabel("2026-07-10", now), "오늘 · 금요일");
  assert.equal(formatMemoDayLabel("2026-07-09", now), "어제 · 목요일");
  assert.equal(formatMemoDayLabel("2026-07-08", now), "7월 8일 수요일");
  assert.equal(formatMemoDayLabel("", now), "");
});
...
test("mapRepliesToThread 는 각 메시지에 dayStamp 를 싣는다(구분선 렌더 근거)", () => {
  ...
  const msgs = mapRepliesToThread(replies as any, "u1");
