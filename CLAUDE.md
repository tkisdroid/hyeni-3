...
- ★★**어른 모드 디자인 언어 정본(2026-08-21 TK 확정 — 이 항목이 아래 시각 사양들을 대체한다)**:
  ...
  · **화면 헤더도 바닥과 같은 문제였다(2026-08-21 TK 제보 "바로가기 → 알림 상단바 깨짐")**:
    ...
    ⚠️ 마스크가 헤더 아래 26%를 지우므로 공용 규칙이 `padding-bottom: 20px` 을 함께 준다 —
    ...
    정본은 `calc(100% - 20px)` 두 곳(배경 그라데이션·mask)이다.
  ...
  · **sticky 헤더에 둘째 줄을 넣지 않는다(2026-08-21 TK 지시)**: 알림 화면의 필터
    (전체·안전·위치)를 헤더에 붙였더니 그 경계에서 목록 카드가 반쯤 비쳤다. 필터는
    **메인 섹션(`.hy-content`) 안 첫 항목**으로 두는 것이 정답이다 — 경계 자체가 사라진다.
