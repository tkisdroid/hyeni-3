...
**v1.4.6/code 18 해외 9개국 Play 출시 대상 확대(2026-09-01, 검토 다시 시작·게시 전)**:
사용자 지시에 따라 production 국가/지역을 대한민국 1개에서 `KR/JP/TW/HK/SG/VN/TH/ID/MY/PH` 총 10개로
확대했다. Play Console 저장 readback은 `국가/지역 10개`, 게시 개요는
`9개 국가/지역 추가: 대만, 말레이시아 외 7개`였다. 2026-09-01부터 진행 중이던 code 18 검토가 취소 후
다시 시작되어 대기 시간이 늘어날 수 있다는 경고를 확인한 뒤 `검토 다시 시작`을 확정했다. 빠른 자동 검사가
끝난 fresh readback은 국가 변경을 포함해 `변경사항을 검토 중입니다`다. 관리형 게시는 꺼져 있어 승인 뒤
자동 게시될 수 있으나 검토 통과·실제 해외 게시는 아직 완료가 아니다. 지도 준비 검사는
`READY_FOR_EXTERNAL_VALIDATION`, locale listing 검사는 10개·위반 0건이지만 운영 release 검사는
시간대/DST·Routes OAuth scope 실호출·비한국 부모 PWA↔아이 Android E2E 세 게이트 때문에 계속 `HOLD`다.
사용자가 이 위험을 감수하고 국가 타기팅 확대를 명시 승인했으므로 Play 배포 대상만 진행했으며, 세 검증이
끝났다고 기록하지 않는다. 상세 정본은 `docs/store/play-console-submission-v1.4.6.md`다.
...
지도 표시가 가능해도 해외 위치 이력·알림 전체가 정확하다고 판정하지 않는다.
...
페어링·refresh token은 변경하지 않았다. 이 배포 시점의 `GOOGLE_MAP_RELEASE_COUNTRIES`는 빈 배열이어서 한국은 Kakao,
그 밖의 국가는 좌표·측정시각 안전 폴백이었다.
