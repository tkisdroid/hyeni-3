...
final class NotificationQuietHoursPolicy {
    ...
    private static final int SEOUL_OFFSET_MINUTES = 9 * 60;
    ...
    private static boolean isUsableSnapshot(NotificationQuietHoursStore.Snapshot snapshot) {
        return snapshot != null
            ...
            && NotificationQuietHoursStore.SEOUL_TIME_ZONE_ID.equals(snapshot.timeZoneId)
            && isMinuteOfDay(snapshot.startMinute)
            && isMinuteOfDay(snapshot.endMinute)
            && snapshot.startMinute != snapshot.endMinute;
    }

    private static int minuteOfDayInSeoul(long nowMs) {
        long utcMinute = Math.floorDiv(nowMs, 60_000L);
        return (int) Math.floorMod(utcMinute + SEOUL_OFFSET_MINUTES, MINUTES_PER_DAY);
    }
